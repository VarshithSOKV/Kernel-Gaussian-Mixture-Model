...
class KernelGMM:
    ...
    def computeKernelMatrix(self, X):
        if self.kernelFunction == "gaussian":
            KernelMatrix = self.gaussian(X, 0.00025)
...
# Show Result
# gmm_map = np.zeros((image_height, image_width, 3))
# cluster_labels_normal = cluster_labels_normal.reshape(image_height, image_width)
# for i in range(image_height):
#     for j in range(image_width):
#         hard_belief = cluster_labels_normal[i][j]
#         gmm_map[i,j,:] = np.asarray(COLORS[hard_belief]) / 255.0
...
for i in range(num_samples-num_classes):
    # Generate two random numbers between 0 and 0.5
    num1 = random.uniform(0, 0.5)
    num2 = random.uniform(0, 0.5)
    ...
    num3 = 1 - num1 - num2
    temp = np.random.randint(0, num_classes)
    P[temp][i+num_classes] = num3
    if temp == 0:
        P[1][i+num_classes] = num1
        P[2][i+num_classes] = num2
    elif temp == 1:
        P[0][i+num_classes] = num1
        P[2][i+num_classes] = num2
    else:
        P[0][i+num_classes] = num1
        P[1][i+num_classes] = num2
...
# with open(file_pathP, 'wb') as f:
#     pickle.dump(P, f)

with open(file_pathP, 'rb') as f:
    P = pickle.load(f)

small_P = np.zeros(num_samples, dtype=int)
for i in range(num_samples):
    small_P[i] = np.argmax(P[:,i])

# init_map = np.zeros((image_height, image_width, 3))
# small_P = small_P.reshape(image_height, image_width)
# for i in range(image_height):
#     for j in range(image_width):
#         hard_belief = small_P[i][j]
#         init_map[i,j,:] = np.asarra
# y(COLORS[hard_belief]) / 255.0
print(-0.0005 * np.sum((X[0] - X[1])**2))
print(np.exp(-0.0005 * np.sum((X[0] - X[1])**2)))
# exit()
...
# kgmm_map = np.zeros((image_height, image_width, 3))
# cluster_labels_kernel = cluster_labels_kernel.reshape(image_height, image_width)
# for i in range(image_height):
#     for j in range(image_width):
#         hard_belief = cluster_labels_kernel[i][j]
#         kgmm_map[i,j,:] = np.asarray(COLORS[hard_belief]) / 255.0
